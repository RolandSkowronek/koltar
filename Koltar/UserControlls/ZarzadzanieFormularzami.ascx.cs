...
namespace stat2018.UserControlls
{
    public partial class ZarzadzanieFormularzami : System.Web.UI.UserControl
    {
        ...
        {
            public stanowiska Stanowiska = new stanowiska();
        ...
        public class stanowiska : common
        {
            public void dodaj(string stanowisko)
            {
                log.Info("dodawanie stanowiska");
                DataTable parametry = makeParameterTable();
                parametry.Rows.Add("@stanowisko", stanowisko);
                runQuerry(" IF NOT EXISTS(SELECT * FROM stanowiska WHERE  stanowisko = @stanowisko)    BEGIN  insert INTO Stanowiska(NAZWA) VALUES  (@stanowisko)    END", con_str, parametry);
            }

            public void usun(string stanowisko)
            {
                log.Info("usuwanie stanowiska");
                DataTable parametry = makeParameterTable();
                parametry.Rows.Add("@stanowisko", stanowisko);

                runQuerry(" delete from stanowiska where   stanowisko = @stanowisko ", con_str, parametry);
            }
        }
